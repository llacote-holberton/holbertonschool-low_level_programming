 ...
 */
int exit_in_error(int error_code, char *tmpl_var)
{
	/* IVD == INVALID */
 ...
 */
int get_source_reader(char *pathname)
{
	int read_source__fd;

	if (!pathname || pathname[0] == '\0')
		exit_in_error(98, pathname);
 ...
 * Return: file descriptor if success. Exit in error otherwise.
 */
int get_dest_writer(char *pathname)
{
	int write_dest__fd;
