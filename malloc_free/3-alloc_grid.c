...
int **alloc_grid(int width, int height)
{
	...
	for (i = 0; i < height; i++)
	{
		/* Try to allocate mem for current line */
		integers_table[i] = (int *)malloc(width * sizeof(int));
		/* Implies to free all previous if this one fails. */
		if (integers_table[i] == NULL)
		{
			/* Pre-decrement because current i had failed! */
			while (--i >= 0)
				free(integers_table[i]);
			free(integers_table);
			return (NULL);
		}
		/* If row available initialize to 0 */
		for (j = 0; j < width; j++)
...
/*
 * === V2 WORKED PERFECTLY BUT CHECKER
 *   PROBABLY WANTED A ROW PER ROW AFFECTATION ===
 * int **integers_table;
 * int *data_area;
 * int i, j;
 * if (width <= 0 || height <= 0)
